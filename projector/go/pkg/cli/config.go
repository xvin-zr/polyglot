...
func getArgs(opts *ProjectorOpts) ([]string, error) {
	...
	if operation == Add {
		if len(opts.Args) != 3 {
			return nil, fmt.Errorf("add requires 2 arguments, but received %v", len(opts.Args))
		}
	...
	if operation == Remove {
		if len(opts.Args) != 2 {
			return nil, fmt.Errorf("remove requires 1 arguments, but received %v", len(opts.Args))
		}
		return opts.Args[1:], nil
	}

	if len(opts.Args) > 1 {
		return nil, fmt.Errorf("print requires 0 or 1 argument, but received %v", len(opts.Args))
	}
...
func NewConfig(opts *ProjectorOpts) (*Config, error) {
	...
	return &Config{Pwd: pwd, Config: config, Operation: getOperation(opts), Args: args}, nil
}
